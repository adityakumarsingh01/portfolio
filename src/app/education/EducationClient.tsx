...
export default function EducationClient({ education }: { education: any[] }) {
  return (
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-4xl">
        ...
        <div className="space-y-8 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-[2px] before:bg-gradient-to-b before:from-transparent before:via-gray-300 dark:before:via-gray-800 before:to-transparent">
          {education.map((item) => (
            ...
            >
              <div className="flex items-center justify-center w-10 h-10 rounded-full border-4 border-[var(--background)] bg-gray-900 dark:bg-white text-white dark:text-gray-900 shadow shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 z-10 text-xl">
                {item.icon}
              </div>
              <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] glass p-6 md:p-8 rounded-[2rem]">
                <div className="flex flex-col mb-2">
                  <span className="text-blue-500 font-mono text-xs uppercase tracking-widest font-bold mb-2">{item.date}</span>
