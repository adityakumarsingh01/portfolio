    ...
    <aside className="fixed left-0 top-0 h-screen w-20 hover:w-64 bg-white dark:bg-[#030014] border-r border-black/10 dark:border-white/10 flex flex-col z-50 transition-all duration-300 overflow-hidden group">
      <div className="p-6 pb-4 flex items-center">
        <Link href="/" className="font-bold text-2xl tracking-tighter flex items-center gap-4 group/logo">
          ...
          <span className="text-gray-900 dark:text-white opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">Aditya<span className="text-[var(--accent-light)]">.</span></span>
        </Link>
      </div>
      <div className="px-6 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap overflow-hidden h-0 group-hover:h-auto">
        <p className="text-xs text-gray-500 font-mono uppercase tracking-widest mb-4">Data Scientist</p>
      </div>
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link 
              key={item.name} 
              href={item.path}
              className={`flex items-center gap-4 px-3 py-3 rounded-xl transition-all duration-200 font-medium text-sm overflow-hidden ${
                isActive 
      ...
      <div className="p-4 border-t border-white/5 flex flex-col group-hover:flex-row gap-4 justify-center items-center">
        ...
        <a href="https://linkedin.com/in/aditya-kumar-singh-990" target="_blank" rel="noreferrer" className="w-10 h-10 shrink-0 rounded-full glass flex items-center justify-center text-gray-500 dark:text-gray-400 hover:text-[var(--accent-light)] hover:bg-black/5 dark:hover:bg-white/5 transition-all hidden group-hover:flex">
